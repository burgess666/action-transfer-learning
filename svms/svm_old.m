    ...
    methods (Static)
        ...
        function output=predict(Model,sample,varargin)
            ...
            if nsample>2
                ...
                for i=1:nsample
                    [~,threshold]=svm_old.svmclassify(model{i},sample,varargin{:});
                ...
                output=svm_old.svmclassify(model,sample,varargin{:});
            end
        ...
        function [outclass,val] = svmclassify(svmStruct,sample, varargin)
            ...
            if ~isempty(sample)
                ...
                %     try
                [outclass,val] = svm_old.svmdecision(sample,svmStruct);
                %     catch ME
        ...
        function [Model,predicted] = classify(Sample,class,SampleTest)
            Model=svm_old.train(Sample,class);
            predicted=svm_old.predict(Model,SampleTest);
        end
    end
end